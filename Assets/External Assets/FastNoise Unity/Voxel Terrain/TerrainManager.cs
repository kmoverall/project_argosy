...
namespace VoxelEngine
{
	...
	public class TerrainManager : MonoBehaviour
	{
		...
		public int updateFrameTimeCutoff = 5;
		...
		{
			UnloadAllChunks();
			targetTransform.position = new Vector3(0,30,0);
		}
		...
		void LateUpdate()
		{
			...
			float unloadDistanceSq = loadDistanceSq * unloadDistanceModifier * unloadDistanceModifier;
			...
			while (chunkMeshQueue.Count > 0)
			{
				...
				if (updateTimer.ElapsedMilliseconds >= updateFrameTimeCutoff + (chunkMeshQueue.Count >> 5))
					break;
			}
