...
namespace VoxelEngine
{
	public class Chunk
	{
		...
		// Use gradient adjustment to smooth mesh
...
#pragma warning restore 0429
		...
		public void Setup(Vector3i chunkPos, TerrainManager terrainManager)
		{
		...
		private void ReleaseAdjChunks()
		{
			for (int i = 0; i < ADJ_CHUNK_SIZE; i++)
			{
				...
				Chunk invAdjChunk = terrainManager.GetChunk(chunkPos - adjChunkVectors[i]);
		...
		public void FillAdjChunks()
		{
			...
			for (int i = 0; i < ADJ_CHUNK_SIZE; i++)
			{
				adjChunks[i] = terrainManager.GetChunk(chunkPos + adjChunkVectors[i]);
			...
			if (dirtyMesh)
				terrainManager.QueueChunkMeshing(chunkPos);
		}
		...
		public bool CheckTerrainBounds()
		{
			...
			if (terrainManager.terrainGenerator.MinHeight() > realPos.y + SIZE)
			{
				FloodVoxelData(terrainManager.terrainGenerator.MinVoxel());
				return false;
			}

			if (terrainManager.terrainGenerator.MaxHeight() < realPos.y)
			{
				FloodVoxelData(terrainManager.terrainGenerator.MaxVoxel());
				return false;
			}
			return true;
		}
		...
		public void GenerateVoxelData()
		{
			dirtyMesh = true;
			fillType = FillType.Null;
			terrainManager.terrainGenerator.GenerateChunk(this);
		}
		...
		public void BuildMesh()
		{
			...
			if (chunkGameObject == null)
			{
				...
				chunkGameObject.Setup(chunkPos, realPos, terrainManager.gameObject.transform);
				chunkGameObject.meshRenderer.material = terrainManager.meshMaterial;
			}
